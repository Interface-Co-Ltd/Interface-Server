...
public class JwtTokenProvider {
    ...
    //JWT 토큰 생성
    public String createToken(String studentId, String auth) {
        ...
        //private claims
        claims.put("studentId", studentId); // 정보는 key - value 쌍으로 저장.
    ...
    public Authentication getAuthentication(String token) {
        User userDetails = userDetailsServiceImpl.loadUserByUsername(this.getUserPk(token));
        return new UsernamePasswordAuthenticationToken(userDetails, "", userDetails.getAuthorities());
    }

    // 토큰에서 회원 정보 추출
    public String getUserPk(String token) {
        return (String) Jwts.parser().setSigningKey(secretKey).parseClaimsJws(token).getBody().get("studentId");
    }
